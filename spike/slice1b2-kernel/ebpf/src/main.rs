...
pub fn signal_return(ctx: RetProbeContext) -> u32 {
    ...
    // 5. winner: timestamp immediately before the single SIGSTOP request. No
    //    winner-side delay, polling loop, or race amplifier may sit between the
    //    CAS and this timestamp/request; check-signal-shape.py proves exactly
    //    that on the frozen object. If the nonwinner's handler is deferred past
    //    the group stop, its record simply never arrives in this owner's
    //    window and the host runs the validated second-owner outcome instead.
    //    Nonwinner: causal timestamp + coalesced status.
