...
- [ ] **Step 3 — verify and classify:** for the absent-destination path, hash every source file before copying and every temporary file after it with a sorted, path-stable manifest; for the existing-destination path, verify the complete exact set/hashes before accepting the no-op. Record exact before/after SHA-256 values and the manifest hash. Verify the two named reports are non-empty and match the tarball cross-check byte-for-byte. Run a private-data/privacy scan and classify PID, address, and raw-capture content as private-only; classify any such content, forbidden paths, or a public SDD recreation as forbidden in the public diff.
...
- [ ] **Step 7 — gates and privacy fence:** despite the docs/custody-only public diff, explicitly run all four canonical gates on the unprivileged path (`cargo +1.88 fmt --all -- --check`, `cargo +1.88 check --locked --workspace --all-targets`, `cargo +1.88 test --locked --workspace --all-targets`, and `cargo +1.88 clippy --locked --workspace --all-targets -- -D warnings`). Verify that a fresh public clone cannot stage the private trove, and scan both repositories' diffs for PIDs, addresses, raw captures, and forbidden paths. Privileged/container rows remain UNRUN without separate approval and do not block the W1 unprivileged exit.
