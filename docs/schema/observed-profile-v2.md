...
| `skipped` | Everything discovery could not use, and why, at every granularity it works at. **Entries**: a NULL table slot, an entry whose object could not be pinned, an unresolvable manifest record. **Objects**: a mapping with no usable pathname (a memfd, or a file deleted under a running process), exports that could not be read, an object over the byte caps, a snapshot that ended early on a read error, and a module the scan classified as a provider but decoded no table in. **Processes and scope**: a process that could not be scanned, a cgroup directory that could not be listed or whose `cgroup.procs` could not be read, and the `MAX_SCAN_PIDS` cap when a `--cgroup` holds more processes than one capture scans. An object- or process-level skip is often the *only* record of a provider that was never decoded at all — it contributes no table, so nothing else in the document would show it. Identical losses are deduplicated, so one loss seen through many processes of a `--cgroup` is one entry; two *different* losses of one subject stay two, and one unreadable cgroup directory legitimately yields two (its `cgroup.procs` and its listing failed separately). The `name` is therefore a function name, an object path, a `pid <n>`, or a cgroup path, according to what was lost; the `reason` says which. |
