...
- [ ] **Step 2 — exhaustive no-clobber state machine:** use the complete source under `/home/user/.local/state/p11scope/retired-generated-slice1b2-finish/.superpowers/sdd/2026-08-27-task4-receipt-closure/` only as input. If the destination `/home/user/src/m/p11scope-ws/preserved/sdd/2026-08-27-task4-receipt-closure/` is absent, copy the source only to a private sibling temporary directory under `p11scope-ws` and proceed to verification. If the destination exists and has complete exact manifest/set/hash equality, perform an idempotent no-op with no copy, merge, or publish. If it exists but is partial, differs, or cannot be completely verified, stop before mutation: never merge, overwrite, or attempt a repair in place.
- [ ] **Step 3 — verify and classify:** for the absent-destination path, hash every source file before copying and every temporary file after it with a sorted, path-stable manifest; for the existing-destination path, verify the complete exact set/hashes before accepting the no-op. Record exact before/after SHA-256 values and the manifest hash. The tar cross-check provenance names `b86d4d5` and the exact archive `/home/user/.local/state/p11scope/pkcs11-scope-portable-b86d4d5.tar.zst` SHA-256 `09da43704d6642d119be20700b0ce30be659847bc112092d09276fb9602f9fc5`; `3d3ba05` may be an additional matching cross-check only, never a substitute. Verify the two named reports are non-empty and match that provenance byte-for-byte. Run a private-data/privacy scan and classify PID, address, and raw-capture content as private-only; classify any such content, forbidden paths, or a public SDD recreation as forbidden in the public diff.
- [ ] **Step 4 — publish without replacement:** only for a verified absent-destination temporary tree, publish with `mv -T --no-clobber` (or an equally minimal native no-replace operation), then assert the destination's exact file set and hashes and that no existing destination was overwritten. Update the evidence index to point to `p11scope-ws/preserved/sdd/2026-08-27-task4-receipt-closure/` and state that no public SDD copy exists; the portable package may hold only the explicitly approved three-file `stage3a3/` subset.
...
- [ ] **Step 6 — private restore contract and reversible mitigations:** the existing private-custody review must state that materialization runs under `umask 077`, asserts mode `0700` on the private root `/home/user/src/m/p11scope-ws` and each exact custody directory, and asserts mode `0600` on each exact custody file. It must explicitly use no hook, dependency, `core.fileMode`, or `.gitattributes` machinery. Record before/after `stat` for the two reversible exposure mitigations, then run `chmod 0700 /home/user/p11scope-vm-bases` (expected before mode `0755`) and `chmod 0600 /home/user/.local/state/p11scope/pkcs11-scope-portable-b86d4d5.tar.zst` (expected before mode `0664`); assert both after modes and make no deletion or replacement of either path.
- [ ] **Step 7 — explicit staging and separate reviews:** stage only an explicit path list (never `git add -A` or a broad glob): public pointer/fence paths in the public repository, and durable private text/manifests in `p11scope-ws`; never stage raw captures or ignored binaries. Review and commit the public pointer/fence separately from the private custody commit, with exact before/after hashes and mode stats in each review record.
- [ ] **Step 8 — gates and privacy fence:** despite the docs/custody-only public diff, explicitly run all four canonical gates on the unprivileged path (`cargo +1.88 fmt --all -- --check`, `cargo +1.88 check --locked --workspace --all-targets`, `cargo +1.88 test --locked --workspace --all-targets`, and `cargo +1.88 clippy --locked --workspace --all-targets -- -D warnings`) at the exact public Task 0 revision. Verify that a fresh public clone cannot stage the private trove, and scan both repositories' diffs for PIDs, addresses, raw captures, and forbidden paths. Privileged/container rows remain UNRUN without separate approval and do not block the W1 unprivileged exit.
- [ ] **Step 9 — final private amendment receipt:** after those four gates finish at the exact public Task 0 revision, write a concise command/revision/exit-status receipt into committed private custody text, with no full logs and no generated public artifacts. Stage only that explicit private path, review it, and make the final private amendment commit after the gate run; if the public pointer/fence changes, rerun the gates and bind a new receipt.
...
Also add these cheap RED tests before touching the parser or capture loop: `default_trace_bound_resolver_none_is_10m` directly calls the resolver with `None` and asserts `10_000_000` (it must not synthesize 10,000,001 events); `stdout_truncation_with_max_events_one` runs a deterministic fixture that emits only two events with `--max-events 1` and asserts one event plus `TRUNCATED`/partial evidence; `bounded_file_trace_is_cumulative` uses the same small fixture and bound across `-o` rather than separately per sink; and `owned_child_limit_reaches_settlement_and_reap` uses `--max-events 1`, reaches terminal drain, emits truncation/evidence, and verifies the child has been reaped. Keep every fixture small and deterministic; no test may generate 10,000,001 events.
...
- [ ] **Step 2: Run every RED test and record the failures before implementation.** Run the CLI parser test, the evidence test below, `default_trace_bound_resolver_none_is_10m`, `stdout_truncation_with_max_events_one`, `bounded_file_trace_is_cumulative`, and `owned_child_limit_reaches_settlement_and_reap` in one focused command (or equivalent individually); each must fail for the missing behavior before any production edit. The default-bound resolver assertion is direct and cheap; the stdout case uses `--max-events 1`, never a 10,000,001-event workload.
...
- [ ] **Step 6: Run focused tests and verify PASS.** Run parsing, evidence, the direct `None → 10_000_000` resolver test, `--max-events 1` stdout truncation, bounded file trace, and the owned run. The default path must be bounded on stdout as well as `-o`, and `CaptureEnd::LimitReached` must still produce terminal evidence and a reaped child; no test generates 10,000,001 events.
...
- [ ] **Step 1: Write failing tests** in `src/discovery/scan.rs` mod tests (model on `dense_candidates_and_interfaces_stop_at_capture_caps:1134`; use an injectable maps-reader seam): `adversarial_near_misses_and_a_512_table_tail_stop_at_the_computation_ceiling` (4096 candidates each a full-104-field near miss rejected at the last field; assert `tables.is_empty()` and `skipped == vec![WORK_CEILING_REASON]`); `maps_reader_crosses_64_mib_without_a_per_object_ceiling` (one maps snapshot crossing 64 MiB still reads until the total budget or `MAX_MAPS_BYTES`, proving no `per_object_bytes` allowance); `maps_exact_cap_is_not_truncated` (reader returns exactly `MAX_MAPS_BYTES`, assert no byte-cap reason); `maps_cap_plus_one_is_truncated_at_the_last_newline` (reader returns `MAX_MAPS_BYTES + 1`, assert only complete lines before the byte cut and `MAPS_CEILING_REASON`, with no hard error); `maps_entry_ceiling_is_independent_below_byte_cap` (an injected reader returns more than `MAX_MAP_ENTRIES` complete newline-terminated records while remaining below `MAX_MAPS_BYTES`, assert only the first `MAX_MAP_ENTRIES` records are parsed and `MAPS_ENTRY_CEILING_REASON` flows through PARTIAL); `maps_entry_and_byte_caps_are_separate` (a result crossing both limits emits both reasons without allowing either cap to suppress the other); `maps_total_byte_exhaustion_is_partial` (exhaust `total_bytes` during the injected maps read, assert `IO_CEILING_REASON` and the existing PARTIAL path); `deadline_interrupts_between_64k_chunks` (deadline becomes expired after a chunk, assert `SCAN_DEADLINE_REASON`); `deadline_is_cleared_after_error_and_success` (subsequent scan has no stale deadline); and `unsorted_maps_fail_closed` (construct reversed intervals, assert the scan refuses instead of using a linear fallback).
- [ ] **Step 2: Run, verify FAIL** (scan currently completes / near misses cost zero budget / the maps read is uncapped).
- [ ] **Step 3: Implement (a) `MapIndex`** in `maps.rs`: extract `:204-222` into `resolved_for`; build a sorted-interval `partition_point` index once after `parse_maps`, and reject unsorted intervals immediately (delete the unsorted linear fallback). **(b)** HashMap `by_address` in `scan_interfaces` replacing the linear `position`. **(c)** add an independent work counter with `const DEFAULT_WORK_CEILING: u64 = 16 * 1024 * 1024`; it is test-overridable and never derived from `total_bytes` (there is no `per_object_bytes` allowance). Charge at each window and inside `decode_candidate`'s field loop before `resolve`; on exhaustion return `WORK_CEILING_REASON`. A `charge(1)` against the O(log n) index is the unit definition. **(d)** add `deadline_ns: Option<u64>` + `set_deadline`; check immediately on entry to each scan, then every 4096 windows via `attach::monotonic_ns()`. For the maps reader, read in bounded 64 KiB chunks through `Read::take(MAX_MAPS_BYTES + 1)`, check the deadline between chunks, and charge every returned byte against the total I/O budget. On total exhaustion return `IO_CEILING_REASON`; do not reset or split that charge per object. Reading `MAX_MAPS_BYTES + 1` distinguishes exact EOF from cap-plus-one. Independently count complete newline-terminated entries on every reader result, retain only the first `MAX_MAP_ENTRIES` records, and whenever another complete entry is present emit `MAPS_ENTRY_CEILING_REASON` through PARTIAL—even when the byte cap is not reached. Separately, if the extra byte exists, trim at the last complete newline before generic parse and emit `MAPS_CEILING_REASON`; both reasons may be emitted when both limits are crossed. Perform entry truncation before calling the unchanged generic parser. **(e)** thread `deadline` through `apply_discovery_batch(_with)` and set it on entry. Clear it on every exit, including all early returns (a guard/finally closure is acceptable); `pause.rs:2076` passes the held deadline already. **(f)** push all reason constants through the existing `Vec<String>` returns. There is no weaker deadline fallback.
...
**Entry/byte-cap distinction (implementation correction):** count complete
newline-terminated entries on every maps-reader result, retain only the first
`MAX_MAP_ENTRIES`, and emit `MAPS_ENTRY_CEILING_REASON` through PARTIAL as soon
as another complete entry is present—even below `MAX_MAPS_BYTES`. Separately,
the `MAX_MAPS_BYTES + 1` extra byte triggers the byte-cap trim at the last
complete newline and `MAPS_CEILING_REASON`; both reasons may be emitted. Apply
entry truncation before the unchanged generic parser. Both triggers are
independent.
...
The preflight is self-contained and exact: **A1** requires full `git status --porcelain=v1 --untracked-files=all` cleanliness (tracked, staged, and untracked); **A2** searches the exact repository-ancestor sequence from the worktree to `/` for each `.cargo/config.toml` and `.cargo/config`, then the effective `CARGO_HOME/config.toml` and `CARGO_HOME/config`, refusing any untracked or unexpected config input; **A3** rejects any non-empty inherited value for exactly these ten variables before the body: `RUSTFLAGS`, `CARGO_ENCODED_RUSTFLAGS`, `CARGO_TARGET_DIR`, `CARGO_BUILD_TARGET`, `CARGO_HOME`, `RUSTUP_HOME`, `RUSTUP_TOOLCHAIN`, `RUSTC_WRAPPER`, `CC`, and `CFLAGS`. After that refusal gate, only plan-controlled command-local values may be supplied, and all ten are rechecked at finalization; **A4** records the exact nine tools `cargo`, `docker`, `file`, `jq`, `python3`, `rustup`, `setpriv`, `sudo`, and `sha256sum`, plus the `rustup`-reported `cargo`/`rustc` toolchain binaries, and every invocation uses its resolved absolute non-symlink path; **A5** permits host build execution only with `--offline` (container/privileged commands retain their separate owner gates). Initial and final repository/tool/environment hashes must match; PATH replacement or mismatch is refusal, never a warning.

- [ ] **Step 1: Write failing tests.** (a) Real-process A2 refusal in `tests/artifact_contracts.rs` (pattern of :811, PATH tripwires from :2512-2543): use a temporary `HOME` containing `.cargo/config.toml`, explicitly unset inherited `CARGO_HOME`, run `build-release.sh <absent_root>` → `exit 77`, stderr contains `"untracked cargo config"`, tripwire log absent (never reached cargo/sudo/rm). (b) A3 table-driven test iterates all ten inherited variable names with a non-empty value and asserts pre-body refusal. (c) Self-test model rows: add `untracked-build-input-rejected-status-77-no-touch-before-body` to `LANE14_CASES` (:2429) AND the `lane` heredoc (:105-122); modeled on `root-preflight-blocks-body-cargo-runtime` (:62). (d) Add refusal rows for replacing one recorded tool between preflight and finalization, and for a PATH change that resolves a different binary; both must fail closed. (e) Static assert: extend `official_build_is_safe_only:709` for `--offline`.
- [ ] **Step 2: Run, verify FAIL.**
- [ ] **Step 3: Implement** in `task4_receipt_run`, all before `command -v`(:295)/`sudo -n`(:296)/`release_body`(:305), refusal `exit 77` (POSIX sh; `sh -n` gate at :2267): implement self-contained A1–A5 exactly as listed above, reject all non-empty inherited A3 values before the body, then invoke commands only with plan-controlled command-local values and record the ten-variable snapshot. Resolve each recorded tool to an absolute non-symlink path and invoke those exact paths (including the rustup-selected cargo/rustc binaries), and record `task4_fact` path/hash rows. At finalization, recheck all ten inherited-variable values are still empty outside command-local scopes and rehash every recorded tool path; refuse publication on any mismatch or replacement. A PATH change that resolves a different binary is a refusal, not a warning. Add `--offline` to the host build only (:335-336). New receipt fields remain additive TSV rows.
...
- [ ] **Step 3:** After the closeout is written, committed, gated, and reviewed on the branch, merge locally to `main` — the owner pre-approved each wave's local merge at review-to-zero (2026-09-01), so no integration pause is needed; pushing, tagging, and publishing stay owner-gated. After the merge, rerun all gates on the merged tip; package the portable bundle/evidence archive only after those merged-tip gates pass. Never package from an unmerged branch tip.
