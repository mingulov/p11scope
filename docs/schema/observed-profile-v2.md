...
| `modules[].dev` | `[major, minor]` | Two unsigned 64-bit integers: the device of the object, as `/proc/<pid>/maps` renders it. |
| `modules[].ino` | integer | Nonzero unsigned 64-bit inode of the object. |
| `modules[].sha256` | string or `null` | Whole-file digest as exactly 64 lowercase hexadecimal characters, taken once at pin time. `null` — never `""` — when nothing pinned the object, so no digest was ever taken; that module was discovered but never authorized, and the capture cannot have probed it. |
...
| `skipped` | Everything discovery could not use, at every granularity it works at. **Entries**: a NULL table slot, an entry whose object could not be pinned, an unresolvable manifest record. **Objects**: a mapping with no usable pathname, exports that could not be read, an object over the byte caps, an incomplete snapshot, or a provider with no decoded file-backed table. **Processes and scope**: a process or cgroup subtree that could not be scanned, and the `MAX_SCAN_PIDS` cap. Such a skip may be the only record of a provider that was never decoded. Identical internal losses are deduplicated before rendering; two different losses remain two records even if sanitization gives them the same public pair. The public `{name, reason}` is finite: `name` is an exact standard function or `discovery subject`; `reason` is exactly `null pointer`, `function entry unavailable`, `function table unavailable in file-backed data`, `shared-overlay physical identity is uncertain; a distinct byte-identical instance may be unobserved`, or the fail-closed `discovery unavailable`. Arbitrary mapped paths, numeric PIDs, `/proc/<pid>` paths, cgroup paths, unknown names, and raw error chains remain diagnostic-only. Every record still forces `PARTIAL`. |
...
| `manifest_object_fallbacks[]` | Stale optional-manifest objects ignored only after one exact scan-opened replacement table covered every dropped function claim. Each record is path/PID-free: `{manifest, object, reason, replacement}`, where `manifest` is an unsigned 32-bit ordinal, `object` is in `0..512`, `reason` is exactly `open_stale` or `identity_mismatch`, and `replacement` is the scan-owned `{dev, ino, sha256}` also present in `discovery[]`. A replacement identity is unique across the full array, including different manifest ordinals. At most 512 records. Each record contributes one to `discovery_uncorroborated` and forces `PARTIAL`. A missing, ambiguous, incomplete, or final-plan-absent replacement is fatal rather than published. |
