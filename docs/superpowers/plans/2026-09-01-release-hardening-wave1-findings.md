...
- [ ] **Step 5:** Make the initial commit in `/home/user/src/m/p11scope-ws` (the repo is git-initialized but has ZERO commits — `evidence/`, `preserved/`, `incoming/` are all untracked; "custody" with no git history is one `rm -rf` from unrecoverable). Commit everything currently there plus the Step 1/4 mirrors.
...
- [ ] **Step 3:** Update memory: mark wave 1 closed, record the new tip and package hash, note waves 2+ still pending: the spec §4 storage migration (two-directory rule — every tracked absolute path off the old roots), research #1 (hardcoded tracepoint offsets, `crates/ebpf/src/main.rs:1703,1706`) and #2 (opened file never verified against the maps-line inode) — both inside spec §5/§6's definition of done and NOT covered by this wave — then CI, containers, multi-distro, 32-bit, all gated on the eBPF-pitfalls research report.
