...
| `CAP_BPF` + `CAP_PERFMON` (no `CAP_SYS_ADMIN`) | **Measured post-fix6** | On this host, 2026-08-25 at `2494fa9`: `exit_status: 0`, `attached_probes: 0/136`, 68 per-slot `attach_failures`, every one containing `` \`perf_event_open\` failed: Permission denied ``; `completeness: PARTIAL`; three sanitized public discovery pairs were recorded. Since main commit `1d3837b` ("fix: attach initial provider exports before readiness") this row records **four** sanitized skips — the ptrace-limited scan, tracefs lifecycle, loader arming, and the pre-readiness export hooks, whose loss under these capabilities was previously unrecorded rather than absent; the three above stay as the dated `2494fa9` measurement. |
