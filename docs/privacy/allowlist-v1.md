...
| PID/TID | `allowlisted` and unsafe diagnostic | `bpf_get_current_pid_tgid`; no user-memory read. | Raw PID/TID in bounded `trace` output; profile/metrics do not publish it. |
| Cgroup id | All modes | `bpf_get_current_cgroup_id`; no user-memory read. | Numeric id and best-effort host cgroup label. |
| Discovery skip subject | All modes | Scan, pinning, process, and scope losses share one untyped internal subject after aggregation. Only exact standard PKCS #11 function names survive capture serialization; every other subject fails closed to the literal `discovery subject`. | `evidence.skipped[].name`; never an arbitrary mapped path, numeric PID label, cgroup path, or unknown name. The reason and number of distinct losses remain intact. |
